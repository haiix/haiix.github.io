...
async function* cursorWrapper<T>(req: IDBRequest<T>): AsyncGenerator<NonNullable<T>, void, unknown> {
  while (true) {
...
export class Idb {
  ...
  private async transaction(storeNames: string | Iterable<string>, mode: IDBTransactionMode, fn: (tx: IDBTransaction) => void): Promise<void> {
  ...
  deleteObjectStore(name: string): void {
    this.upgReqs.push([name, 'delete', (db) => {
    ...
    if (this.upgReqs.length > 0) {
      ...
      for (const [storeName, method] of this.upgReqs) {
        if (method === 'create' && !currentObjectStores.contains(storeName)) {
          reqUpdate = true;
        } else if (method === 'delete' && currentObjectStores.contains(storeName)) {
          reqUpdate = true;
        }
      }
      if (reqUpdate) {
...
abstract class IdbStoreBase {
  ...
  async getAll(query?: IDBValidKey | IDBKeyRange | null, count?: number): Promise<unknown[]> {
    return this.register('readonly', (os) => queryWrapper(os.getAll(query, count)));
  }

  async getAllKeys(query?: IDBValidKey | IDBKeyRange | null, count?: number): Promise<IDBValidKey[]> {
    return this.register('readonly', (os) => queryWrapper(os.getAllKeys(query, count)));
  }

  async getKey(query: IDBValidKey | IDBKeyRange): Promise<IDBValidKey | undefined> {
    return this.register('readonly', (os) => queryWrapper(os.getKey(query)));
  }

  async *openCursor(query?: IDBValidKey | IDBKeyRange | null, direction?: IDBCursorDirection): AsyncGenerator<IDBCursorWithValue, void, unknown> {
    yield* await this.register('readwrite', (os) => cursorWrapper(os.openCursor(query, direction)));
  }

  async *openKeyCursor(query?: IDBValidKey | IDBKeyRange | null, direction?: IDBCursorDirection): AsyncGenerator<IDBCursor, void, unknown> {
    yield* await this.register('readwrite', (os) => cursorWrapper(os.openKeyCursor(query, direction)));
  }
}
...
export class IdbStore extends IdbStoreBase {
  ...
  constructor(db: Idb, name: string, options?: IDBObjectStoreParameters, indices?: IndexParameters[]) {
    ...
    this.db = db;
    this.name = name;
    db.upgReqs.push([name, 'create', (db) => {
  ...
  protected register<T>(mode: IDBTransactionMode, fn: (os: IDBObjectStore) => T): Promise<T> {
    return new Promise((resolve, reject) => {
      this.db.reqs.push([this.name, mode, (tx) => {
        try {
  ...
  async add(value: unknown, key?: IDBValidKey | undefined): Promise<IDBValidKey> {
  ...
  async put(value: unknown, key?: IDBValidKey | undefined): Promise<IDBValidKey> {
...
export class IdbIndex extends IdbStoreBase {
  ...
  protected register<T>(mode: IDBTransactionMode, fn: (os: IDBIndex) => T): Promise<T> {
    return new Promise((resolve, reject) => {
      this.db.reqs.push([this.storeName, mode, (tx) => {
        try {
