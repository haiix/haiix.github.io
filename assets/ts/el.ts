...
export function h1(
  ...
  return text(label, classList, ref ?? 'h1') as HTMLHeadingElement;
}
...
export function h2(
  ...
  return text(label, classList, ref ?? 'h2') as HTMLHeadingElement;
}
...
export function h3(
  ...
  return text(label, classList, ref ?? 'h3') as HTMLHeadingElement;
}
...
export function p(
  ...
  return text(label, classList, ref ?? 'p') as HTMLParagraphElement;
}

function setAttrs(element: Element, attrs?: Record<string, string | number | boolean | null | undefined | Function> | null) {
  if (!attrs) return;
...
export function input(
  ...
  const element = container(classList, ref ?? 'input') as HTMLInputElement;
  setAttrs(element, { type, ...props });
  return element;
}
...
export function textarea(props?: TextAreaProps | null, classList?: string | null, ref?: HTMLTextAreaElement) {
  const element = container(classList, ref ?? 'textarea') as HTMLTextAreaElement;
  setAttrs(element, { ...props });
  return element;  
}

let globalLabelIdCount = 0;
export function label(labelText: string, props?: { for?: string | Element } | null, classList?: string | null, ref?: HTMLLabelElement): HTMLLabelElement {
  const element = text(labelText, classList, ref ?? 'label') as HTMLLabelElement;
  const forElem = props?.for;
  if (typeof forElem === 'string') {
    element.setAttribute('for', forElem);
  } else if (forElem) {
    forElem.id ||= `el-global-label-id-${++globalLabelIdCount}`;
...
export function image(
  ...
  const element = container(classList, ref ?? 'img') as HTMLImageElement;
  element.alt = alt;
  return element;
}
